...
#
# May be freely used and copied providing this notice is retained No warranty, use at your own risk
#
# valgrind foo 3>&1 1>&2 2>&3 | grindmerge -f input >newrules
use warnings; use strict; use Digest::MD5 qw/md5_hex/; my %known; my $dups = 0; sub parse {
    my $in = shift;
    while( <$in> ) {
        next unless( /^\{/ );
        my $block = $_;
...
parse( \*STDIN ); print $known{$_} foreach ( sort keys %known );
